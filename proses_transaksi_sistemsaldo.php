...
if($op=='ambilbarang'){
    ...
    if($del){
        echo "<script>window.location='index.php?page=transaksi_sistemsaldo';</script>";
    }else{
        echo "<script>alert('Hapus Data Berhasil');
            window.location='index.php?page=transaksi_sistemsaldo';</script>";
    }
    ...
    $simpan=mysql_query("insert into data_transaksi_sistemsaldo(id_nota,id_pel,tgl,kredit,debit,sisa_saldo)
                        values ('$nota','$id_pel','$tgl','$tot','$debit','$sisa_saldo')");
			
    if($simpan){
        $query=mysql_query("select * from data_cucian");
        while($r=mysql_fetch_row($query)){
            mysql_query("insert into data_detailtransaksi_sistemsaldo(id_nota,id_paket,harga,banyak,subtotal)
